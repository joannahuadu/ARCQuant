...
def main():
    ...
    print("Start bf16 JOINT calibration (teacher = hook disabled)...")
    for layer_idx in range(len(layers)):
        ...
        for param in trainable_gate:
            ...
            for xm in iter_layer_x_mask_modules(layer):
                if args.trainable_gate and param is getattr(xm, "x_mask_gate_logits", None):
                    owner_ok = True
                mlp_mod = getattr(xm, "x_mask_token_mlp", None)
                if args.trainable_token_gate and mlp_mod is not None and any(param is p for p in mlp_mod.parameters()):
                    owner_ok = True
                if args.trainable_token_gate and x_mask_token_use_layer_scale and param is getattr(xm, "x_mask_token_scale", None):
                    owner_ok = True
                if args.reset_gate_logits and param is getattr(xm, "x_mask_gate_logits", None):
                    with torch.no_grad():
                        param.zero_()
            if owner_ok:
        ...
        optimizer = torch.optim.AdamW(param_groups) if param_groups else None
        ...
        if optimizer is not None:
            del optimizer
        del trainable_gate, trainable_alpha
        for xm in iter_layer_x_mask_modules(layer):
            for attr in list(vars(xm)):
                if attr.startswith("_last_x_mask"):
                    setattr(xm, attr, None)
    ...
    out_path = os.path.join(args.exp_dir, f"{model_name.lower()}_joint_{args.dataset}_bf16.pt")
